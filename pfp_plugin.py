...
def _msg(char, msg, pre="[", post="]", color=None):
    ...
    for line in msg.split("\n"):
        print(
            "{pre}{char}{post} {line}".format(
                pre=pre, char=char, post=post, line=line
            )
        )
...
def create_scratch(
    ...
    command = "silent keepalt botright vertical {width}split {name}".format(
        width=max_line_width, name="PFP_DOM"
    )
    vim.command(command)
    count = 0
...
def pfp_choose_template():
    ...
    for template_dir in template_dirs:
        templates += glob.glob(
            os.path.join(os.path.expanduser(template_dir), "*.bt")
        )

    templates = sorted(templates, key=lambda s: s.lower())
...
def pfp_hex_dump_file(filename):
    ...
    with open(filename, "rb") as f:
        ...
        while True:
            ...
            hex_lines.append(
                "{:04x} {} {}".format(count * 0x10, formatted, printable)
            )
            count += 1
            if len(data) != 0x10:
                break
...
def pfp_highlight(start_line, end_line, start_col, end_col):
    vim.eval(
        "matchadd('pfp_hex_selection', '\\%>{start_line}l\\%<{end_line}l\\%>{start_col}c\\%<{end_col}c')".format(
...
def pfp_hex_match_range(s_offset, e_offset):
    ...
    """
    if s_offset % 0x10 != 0:
        if pfp_hex_allign_y(s_offset) == pfp_hex_allign_y(
            e_offset
            ...
            pfp_hex_match_range(
                (s_offset | 0xF) + 1, e_offset
            )  # calls itself to procede drawing further than first line
            return
    ...
    if (e_offset + 1) % 0x10 != 0:  # is the end offset alligned?
        ...
        pfp_hex_match_range(
            s_offset, (e_offset | 0xF) - 0x10
        )  # calss itself to match the rest of the area
        return
